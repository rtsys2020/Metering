...
void find_erase_prepare_sector(uint32_t flash_address);
...
uint32_t write_flash(uint32_t * dst, uint8_t * src, uint32_t no_of_bytes)
{
  uint32_t i;
	...
	if( byte_ctr == FLASH_BUF_SIZE)
	{
	  ...
	  find_erase_prepare_sector((uint32_t)flash_address);
      if(result_table[0] != CMD_SUCCESS)
      {
        while(1); /* No way to recover. Just let Windows report a write failure */
      }
      write_data((uint32_t)flash_address,(uint32_t *)flash_buf,FLASH_BUF_SIZE);
      if(result_table[0] != CMD_SUCCESS)
      {
        while(1); /* No way to recover. Just let Windows report a write failure */
      }
